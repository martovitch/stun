...
#################################
log "3. Проверка DNS и получение SSL сертификата..."

# --- ИСПРАВЛЕНО: Принудительно определяем только IPv4 этого сервера ---
CURRENT_IP=$(curl -s -4 ifconfig.me || echo "unknown")
# Узнаем, куда указывает домен (берем последнюю A-запись)
RESOLVED_IP=$(dig +short "$DOMAIN" A | tail -n1)

log "Ваш IPv4: $CURRENT_IP | IP домена $DOMAIN: $RESOLVED_IP"

# Разрешаем порты для проверки Certbot
ufw allow 80/tcp >/dev/null
ufw allow 443/tcp >/dev/null

# Логика выбора типа сертификата
if [[ "$RESOLVED_IP" == "$CURRENT_IP" ]]; then
    log "DNS настроен верно. Запускаю Certbot для получения настоящего SSL..."
    certbot certonly --nginx -d "$DOMAIN" --non-interactive --agree-tos -m "$EMAIL" || warn "Certbot не смог получить сертификат автоматически."
    
    # Настройка крона на автопродление (только если сертификат от Let's Encrypt)
...
# Определяем локальный IP для SNAT (берем IPv4)
...
echo "Автопродление SSL: Включено (если выпущен настоящий)"
echo "---------------------------------------------------"
